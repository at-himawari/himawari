...
function createS3MediaLibrary(options) {
    ...
    return {
      ...
      async upload(file) {
        const presignResponse = await fetch(`${apiUrl}?action=upload&fileName=${encodeURIComponent(file.name)}&fileType=${encodeURIComponent(file.type)}`);
        if (!presignResponse.ok) { throw new Error(`Failed to get pre-signed URL: ${await presignResponse.text()}`); }
        const { signedUrl, key, publicUrl } = await presignResponse.json();
  
        await fetch(signedUrl, { method: 'PUT', headers: { 'Content-Type': file.type }, body: file });
  
        return { id: key, name: file.name, url: publicUrl, size: file.size, file };
      },
  
      async delete(mediaFile) {
        if (!mediaFile.id) { throw new Error("File ID is missing."); }
        const deleteResponse = await fetch(`${apiUrl}?action=delete&key=${encodeURIComponent(mediaFile.id)}`);
        if (!deleteResponse.ok) { throw new Error(`Failed to delete file: ${await deleteResponse.text()}`); }
      },
